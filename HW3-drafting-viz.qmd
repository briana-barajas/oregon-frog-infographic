...
6.  Mock up your visualizations using code. We understand that you will continue to iterate on these into HW #4 (particularly after receiving feedback), but by the end of HW #3, you should:
    ...
    **ANS:** Data wrangling, and preliminary-plots below
    ...
    ```{r}
    # need to add arrow pointing at Jackson Creek, OR
    ggplot() +
    ...
    ```{r}
    # need to add annotate with frog counts for different sites
    ...
    ```{r}
    # need to change point to an image, want it to look like a frog jumping
    mf_count <- frogs %>% 
      group_by(sex) %>% 
