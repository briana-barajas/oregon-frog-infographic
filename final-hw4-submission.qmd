...
ggplot(data = mf_count) +
  ...
  labs(title = "Number of Frogs Caughy by Sex") +
...
# Plot annual NDVI for two different sites
ggplot(env, aes(x = year, y = mdNDVI, col = reach,
                linetype = reach)) +
  geom_line(linewidth = 1) +
  
  # update colors (match based on vegetation density)
  scale_color_manual(values = c("chartreuse4", "goldenrod3")) +
  
  # update to auto theme
  theme_minimal() +
  
  # update axis labels/title
  labs(col = "Site Name",
       y = "Average Summer NDVI",
       x = "Survey Year",
       title = "Vegetation Density at Survey Sites") +
  
  # update text fonts and size
  ...
  # add captions on interpreting NDVI
  annotate(geom = "text", x = 2019.5, y = 1.5,
           label = "Middle Jack has more dense \n vegetation than Upper Jamison", family = "noto", size = 5)
...
yellow_pad <- here("yellow-pad.png")
green_pad <- here("green-pad.png")
...
geom_segment(aes(y = middle_jack, yend = upper_jamison,
  ...
  theme(panel.background = element_rect(fill = "aliceblue",
        ...
        plot.title = element_text(family = "roboto", size = 26,
                                  face = "bold", hjust = 0.5),
        axis.title = element_text(family = "noto", size = 16),
        axis.text = element_text(family = "noto", size = 14)) +
  
  # add annotation to provide information for NDVI
  annotate(geom = "text", size = 5, family = "noto",
...
ggplot(data = frogs_subset, aes(x = sul_mm)) +
  ...
  # add an annotation on how to measure frog length
  annotate(geom = "text", x = 76.5, y = 39, family = "noto",
           label = "Snout to urostyle length", size = 7)
