...
# jack creek map
creek_path <- here("data", "jack-creek-map.png")
```
...
map <- ggplot() +
  ...
  geom_sf(data = data_location, shape = 15, size = 6, col = "purple1",
          alpha = 0.45) +
  
  # add arrow pointing to study area
  geom_curve(aes(x = -120, xend = -121.50,
                 y = 44, yend = 43.224875),
             curvature = -0.3, col = "purple1", linewidth = 0.7,
             arrow = grid::arrow()) +
  
  # add zoomed in image of study area
  
  
  # # add annotation
  # annotate(geom = "text", x = -119.6, y = 44.25, label = "Jack Creek\n Study Area", family = "noto", size = 5, col = "purple") +
  
  theme_void() +
  labs(title = "Species Range Map") +
  
  # customize title theme
  theme(plot.title = element_text(hjust = 0.5, family = "roboto", size = 20))

map + inset_element(p = test,
                    left = 0.5, 
                    bottom = 0.2, 
                    right = 0.9,
                    top = 0.7,
                    clip = TRUE) + theme_void()
