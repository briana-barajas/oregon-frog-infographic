...
# jack creek map ----
creek_path <- here("data", "jack-creek.png")
creek_img <- png::readPNG(creek_path, native = TRUE)
...
rm(env_raw, frogs_raw, query, creek_path)
```
...
ggplot() +
  ...
  annotate(geom = "text", x = -118, y = 45.3, label = "Study Area \n Jack Creek, Oregon", family = "noto", size = 5.5, col = "#8A2BE2") +
  
  # expand axis limits so inset image does not get cropped
  coord_sf(xlim = c(-125, -116), ylim = c(41.5, 49.5), expand = FALSE) +
  
  # add arrows connecting study area to inset map
  geom_curve(aes(x = -120, xend = -121.50,
                 y = 44.98, yend = 43.224875),
             curvature = 0, col = "#8A2BE2", linewidth = 0.7) +
