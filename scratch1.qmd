...
frogs <- frogs_raw %>%
  ...
  # filter(reach %in% c("Middle Jack", "Upper Jamison")) %>% 
  ...
  # combine frog_detected so individuals are only counted once per year
  group_by(year, reach, tag, sex, sul_mm, size_group) %>% #group by to keep cols
...
## END HW 2, START HW3 SECTION PREP 
...
GOOD line plot of frogs over the year
```{r}
# calculate proportion of individuals that were found every year 
frog_line <- frogs_env %>% 
...
# add species name
# image different frog sizes
```

NDVI x frog-count line plot
```{r}
ndvi_line <- frogs_env %>% 
...
Bad radar plot (submitted)
```{r, eval = FALSE}
frogs_radar <- frogs %>% ungroup() %>% 
  group_by(sex, year) %>% 
  select(-c("tag", "reach", "sul_mm")) %>%
  summarise(count = sum(frog_detected)) %>% 
  pivot_wider(names_from = year, values_from = count) %>% 
  ungroup() %>% 
  select(-c(sex))

rownames(frogs_radar) <- c("female", "male")
frogs_radar <- rbind(rep(25,5) , rep(0,5) , frogs_radar)
  
radarchart(frogs_radar, 
           pcol = c("#F35C4A", "#A3B18A"), # color line
           plwd = 3, plty = 1, # line width and type
           cglcol = "darkgrey", cglty = 1,
           title = "Annual Frog Count Surveys at Jack Creek, Oregon")[]

legend(x=.9, y=1.2, legend = c("female", "male"),
       bty = "n", pch=20 , 
       col=c("#F35C4A", "#A3B18A") , 
       text.col = "black", cex=1.2, pt.cex=3)

# add count data
# change order of years (but really years isn't conventional)
# add count values for each radial point
```


## Comparing sites (HW3 CONT)

```{r}
frogs %>% 
  group_by(reach, year) %>% 
  mutate(year = as.numeric(year)) %>% 
  summarise(annual_frog_count = sum(frog_detected)) %>%
  filter(annual_frog_count != 0) %>% 
  ggplot(aes(x = year, y = annual_frog_count, col = reach)) +
  geom_vline(xintercept = 2014, linetype = 2) +
...
frogs %>% 
  group_by(reach, year) %>% 
  summarise(annual_count = sum(frog_detected)) %>% 
  
  ggplot() +
  geom_col(aes(x = year, y = annual_count, fill = reach)) +
  facet_wrap(~reach)
```
